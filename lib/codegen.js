...
export default class Codegen {
   ...
   */
  Macro(node) {
    let ctx = est.objpat(node.args.map(n => this.convert(n)))
    let macro = est.generator(ident(node.name), ctx)
  ...
  Scope(node, block) {
    ...
    if (hasVars) {
      ...
      this.push()
      block.let([[this.context, est.object(vars)]])
   ...
   */
  ForLoop(node, block) {
    let loop = est.ident(`__j_loop_${this._inx++}`)
    let iter = est.iterator(this.convert(node.iterable))
    block.let([[loop, iter]])

    let code = block.loop()
    let item = this.convert(node.pattern)
    code.let([[item, est.call(est.member(loop, 'next'))]])

    code.if(est.member(item, est.ident('done'))).then.break()
    code.add(est.set(item, '=', est.member(item, est.ident('value'))))
  ...
  MacroCall(node, block) {
    ...
    let code = est.member(est.this(), est.ident('macros'), ident(node.macro))
    let args = est.object(Array.from(zip(node.args, macro.args), ([arg, spec]) => [
      ident(spec.name), this.convert(arg)
    ]))

    block.add(est.yield(est.call(code, args), true))
  }
