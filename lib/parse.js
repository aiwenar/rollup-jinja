...
export class Parser {
  ...
  if(id) {
    ...
    let { alternatives, template } = this.stack.pop()
    if (template.type === 'IfStatement') {
      template.alternative = null
    }
    template.end = this.source.location

    let node = alternatives.reduceRight((acc, node) => {
      node.alternative = acc
      node.end = acc.end
      return node
    }, template)
    ...
    let { alternatives, template, start } = this.stack.pop()
    let body = []
    template.end = this.source.location
